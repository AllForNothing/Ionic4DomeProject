 ...
 *  4.如果可拖拽元素有margin值，可将其手动设为0;或者设置偏移补偿量来抵消margin
 *    例如<ion-fab [offsetY]="28" appDraggable vertical="center" horizontal="end" slot="fixed">
 *           <ion-fab-button>
 *              <ion-icon name="add"></ion-icon>
 *           </ion-fab-button>
 *        </ion-fab>
 *       在浏览器中审查该元素，可以看到该元素#shadow-root:host下有margin-left:-28px
 *       对于这种封装组件，因为不太好手动将其置为0; 则添加[offsetY]="28"即可轻松解决
 *
 */
...
export class DraggableDirective {
  @Input()
  offsetX: number = 0; // 偏移补偿量,单位为px,使用rem等百分比计量单位的，需转化为px.可用于抵消margin-top
  @Input()
  offsetY: number = 0; // 偏移补偿量,单位为px,使用rem等百分比计量单位的，需转化为px.可用于抵消margin-left
